...
def broydens_method(arm: ArmDriver, target_point: Tuple[float, float]) -> None:
    # move robot joint first
    current_angles = tuple(arm.get_angles())
    J_cur = compute_jacobian(arm.get_lengths(), current_angles)
    ...
    current_angle = [radians(current_angles[0]), radians(current_angles[1])]
    ...
    for target in goal_points:
        current_position = forward_kinematics(arm, (degrees(current_angle[0]), degrees(current_angle[1])))
        ...
        while error_distance > GOAL_TOLERANCE:
            ...
            print((degrees(current_angle[0]), degrees(current_angle[1])))
            # move the robot joints
            arm.set_angles((degrees(current_angle[0]), degrees(current_angle[1])))
